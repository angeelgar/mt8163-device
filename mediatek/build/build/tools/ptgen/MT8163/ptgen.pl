...
=cut
    ...
    $ArgList{PRELOADER_PROJECT}          = $ENV{PRELOADER_TARGET_PRODUCT};
    ...
    $ArgList{MTK_IN_HOUSE_TEE_ATF_SUPPORT}  = $ENV{MTK_ATF_SUPPORT};
    $ArgList{MTK_IN_HOUSE_TEE_TEE_SUPPORT}  = $ENV{MTK_IN_HOUSE_TEE_SUPPORT};
    ...
    $ArgList{INHOUSE_GPU}	             = $ENV{MTK_INHOUSE_GPU};
...
sub ProcessRawPartitionLayoutData
{
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        ...
        {
            if ($ArgList{MTK_IN_HOUSE_TEE_ATF_SUPPORT} ne "yes" && $ArgList{MTK_IN_HOUSE_TEE_TEE_SUPPORT} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
            elsif ($ArgList{MTK_IN_HOUSE_TEE_TEE_SUPPORT} eq "yes")
            {
                $partition_layout_process[$partition_idx]->{Download_File} = "tz.img";
            } 
        }
...
__TEMPLATE
    ...
    if($ArgList{INHOUSE_GPU} eq "yes")
    {
        print $ScatterFileFH "${SecondDashes}config_version${colon}V1.1.3\n";
    }
    else
    {
    ...
    if($ArgList{INHOUSE_GPU} eq "yes")
    {
        print $ScatterFileFH "${SecondSpaceSymbol}ext_id_check${colon}true\n";
        print $ScatterFileFH "${SecondSpaceSymbol}ext_id_reg${colon}0x1300_0020\n";
        print $ScatterFileFH "${SecondSpaceSymbol}ext_id_value${colon}0x0207_5017\n";
    }
