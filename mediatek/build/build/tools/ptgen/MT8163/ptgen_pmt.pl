...
$MTK_IN_HOUSE_TEE_ATF_SUPPORT = $ENV{MTK_ATF_SUPPORT};
...
$MTK_IN_HOUSE_TEE_TEE_SUPPORT = $ENV{MTK_TEE_SUPPORT};
...
my $COMBO_NAND_KERNELH = "$CD_ALPS/kernel-3.4/arch/arm/mach-mt8127/$PROJECT/common/combo_nand.h";
...
my $COPY_PATH_KERNEL_PARTITION_H="$CD_ALPS/kernel-3.4/arch/arm/mach-mt8127/$PROJECT/common/";
...
my $COPY_PATH_PARTITION_DEFINE_H_NAME_K="$CD_ALPS/kernel-3.4/arch/arm/mach-mt8127/$PROJECT/common/";
my $COPY_PATH_PARTITION_DEFINE_C_NAME    = "$CD_ALPS/kernel-3.4/arch/arm/mach-mt8127/$PROJECT/common/";
...
my $COPY_PATH_PMT_H_NAME_K="$CD_ALPS/kernel-3.4/arch/arm/mach-mt8127/$PROJECT/common/";
...
print "PTGEN_PROJECT_OUT=$PTGEN_PROJECT_OUT\n";
...
if($EMMC_SUPPORT eq "yes"){
	$SHEET_NAME = "emmc";
if( ($MTK_IN_HOUSE_TEE_TEE_SUPPORT eq "yes") || ($MTK_IN_HOUSE_TEE_ATF_SUPPORT eq "yes") ){
...
print "SCAT_NAME_DIR=$SCAT_NAME_DIR\n" ;
...
sub ReadExcelFile()
{
		...
		if($type eq "EXT4" || $type eq "FAT" ){
			if($pt_name eq "FAT" && $MTK_SHARED_SDCARD eq "yes"){
				...
				print "Skip CUSTOM because of MTK_CUSTOM_PARTITION_SUPPORT off\n";
					...
					($MTK_IN_HOUSE_TEE_TEE_SUPPORT ne "yes" && $MTK_IN_HOUSE_TEE_ATF_SUPPORT ne "yes")){
				print "Skip TEE1 because of MTK_IN_HOUSE_TEE_TEE_SUPPORT and MTK_IN_HOUSE_TEE_ATF_SUPPORT off\n";
			}elsif($pt_name eq "TEE2" &&
					($MTK_IN_HOUSE_TEE_TEE_SUPPORT ne "yes" && $MTK_IN_HOUSE_TEE_ATF_SUPPORT ne "yes")){
				print "Skip TEE2 because of MTK_IN_HOUSE_TEE_TEE_SUPPORT and MTK_IN_HOUSE_TEE_ATF_SUPPORT off\n";
	...
	while($pt_name ne "END"){
		if($pt_name eq "FAT" && $skip_fat==1 ){
			...
			print "Skip CUSTOM because of MTK_CUSTOM_PARTITION_SUPPORT off\n";
        }elsif($pt_name eq "TEE1" && ($MTK_IN_HOUSE_TEE_TEE_SUPPORT ne "yes" && $MTK_IN_HOUSE_TEE_ATF_SUPPORT ne "yes"))
		{
			print "Skip TEE1 because of MTK_IN_HOUSE_TEE_TEE_SUPPORT and MTK_IN_HOUSE_TEE_ATF_SUPPORT off\n";
        }elsif($pt_name eq "TEE2" && ($MTK_IN_HOUSE_TEE_TEE_SUPPORT ne "yes" && $MTK_IN_HOUSE_TEE_ATF_SUPPORT ne "yes"))
		{
			print "Skip TEE2 because of MTK_IN_HOUSE_TEE_TEE_SUPPORT and MTK_IN_HOUSE_TEE_ATF_SUPPORT off\n";
		}elsif($pt_name eq "PERSIST" && $MTK_PERSIST_PARTITION_SUPPORT ne "yes"){
			print "Skip persist because of MTK_PERSIST_PARTITION_SUPPORT off\n";
...
sub GenYAMLScatFile(){
	...
	my %fileHash=(
        ...
        if ($MTK_IN_HOUSE_TEE_ATF_SUPPORT eq "yes")
        {
	    if ($MTK_IN_HOUSE_TEE_TEE_SUPPORT eq "yes")
	    {
	    ...
	    {
	    	$fileHash{TEE1}="trustzone.bin";
	    	$fileHash{TEE2}="trustzone.bin";
	    }
        }
...
sub do_copy_files_old {
	...
	if (-e "$SCAT_NAME_DIR/${PLATFORM}_Android_scatter.txt") {
		if(-e "$COPY_SCATTER_BR_FILES_PATH/${PLATFORM}_Android_scatter.txt"){
			`chmod 777 $COPY_SCATTER_BR_FILES_PATH/${PLATFORM}_Android_scatter.txt`;	
		}						
		`cp $SCAT_NAME_DIR/${PLATFORM}_Android_scatter.txt $COPY_SCATTER_BR_FILES_PATH`;
		print AUTO_CHECK_OUT_FILES "$COPY_SCATTER_BR_FILES_PATH/${PLATFORM}_Android_scatter.txt\n";
	}
...
sub do_copy_files {
  ...
  if (-e "$SCAT_NAME_DIR/${PLATFORM}_Android_scatter.txt") {
			copy_file("$SCAT_NAME_DIR/${PLATFORM}_Android_scatter.txt",$COPY_SCATTER_BR_FILES_PATH);
	}
	if (-e "$SCAT_NAME_DIR/partition_size.mk") {
