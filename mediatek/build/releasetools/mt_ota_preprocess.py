...
def main(argv):
  ...
  for filename in partition_map:
    ...
    # copy sig file for verify used only
    try:
      src_filename = "%s/sig/%s.sig" % (out_folder, os.path.splitext(filename)[0])
      dst_filename = "%s/META/%s.sig" % (zip_root_folder, os.path.splitext(filename)[0])
      shutil.copy(src_filename, dst_filename)
    except:
      print "[OTA Preprocess] sig file '%s' is not exist" % src_filename
