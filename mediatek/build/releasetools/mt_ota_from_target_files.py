...
def AddSecurity_Items(script, input_zip, output_zip, info_dict):
  # security version
  print "[SEC OTA] Adding security version"
  sec_ver_path = "OTA/SEC_VER.txt"
  print "[SEC OTA] security config : " , sec_ver_path
  try:
    sec_ver = input_zip.read(sec_ver_path)
    common.ZipWriteStr(output_zip, "SEC_VER.txt", sec_ver)
    print "[SEC_OTA] SEC_VER.txt included"
  except:
    print "[SEC_OTA] SEC_VER.txt ignored"

  boot_sec_sig_ext_path = "OTA/boot.img.sig"
  try:
    boot_sec_sig_ext = input_zip.read(boot_sec_sig_ext_path)
    common.ZipWriteStr(output_zip, "boot.img.sig", boot_sec_sig_ext)
    print "[SEC OTA] security boot sig_ext : " , boot_sec_sig_ext_path
  except:
    print "[SEC OTA] security boot sig_ext %s not found" % boot_sec_sig_ext_path
  ...
  try:
    ...
    try:
      fs = info_dict["fstab"]["/boot"]
    except:
      fs = info_dict["fstab"]["/bootimg"]
    script.ApplySig("sig/boot.sig", fs.device)
  except:
    print "[SEC OTA] signature files not found, skip apply signature to boot and recovery"
...
def FullOTA_InstallEnd(self):
  ...
  # add security items
  AddSecurity_Items(script, input_zip, output_zip, info_dict)
...
def IncrementalOTA_InstallEnd(self):
  ...
  # add security items
  AddSecurity_Items(script, target_zip, output_zip, tgt_info_dict)
